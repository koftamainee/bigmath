...
mp_limb_t mpn_submul_1(mp_ptr rp, const mp_limb_t *xp, mp_size_t n, mp_limb_t val) {
  if (val == 0) {
    return 0;
  }
  ...
  for (mp_size_t i = 0; i < n; i++) {
    __uint128_t prod = (__uint128_t)xp[i] * val + borrow;
    mp_limb_t lo = (mp_limb_t)prod;
    borrow = (mp_limb_t)(prod >> 64);
    borrow += (rp[i] < lo);
    rp[i] -= lo;
  }
#else
